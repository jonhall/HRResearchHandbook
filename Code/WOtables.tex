...
\newcommand{\orangebullet}{\textcolor{orange}{\scaletextbullet{3}}}
\newcommand{\redbullet}{\textcolor{red}{\scaletextbullet{3}}}
...
\newcommand{\headers}{%
  Element ({\redbullet} new, {\orangebullet} updated)&Guidance&{Where\\covered}%
}

%% Simple N column table for Writing Outcomes
\NewDocumentEnvironment{SimpleNColTableWO}{
    m                      % #1 label
    m                      % #2 number of columns (currently unused)
    O{\widetablewidth}     % #3 table width (default: \widetablewidth)
    m                      % #4 caption
    O{X[3,l]X[6,l]X[-1,l]} % #5 colspec
    O{white}               % #6 table background colour
    +b                     % #7 body
}{\ShortCrefsInThisTable
  \begin{tblr}[
      long,
      label   = {#1},
      caption = {#4},
      ...
      width   = #3,
      colspec = {#5},
      cells   = {bg=#6}, % Cell background colour
      hline{3,8,12,15,19,24,28,35,36,38,Z} = {2-3}{fg=\frameColor, wd=2pt},
      hline{3,8,12,15,19,24,28,35,36,38} = {1}{fg=#6,          wd=2pt},
      colsep  = 5pt,
       column{1} = {fg=white, bg=\frameColor},
      row{1}    = {font=\bfseries, fg=white, bg=\frameColor},
      rows      = {m},
      rowhead   = 1,
      measure   = vbox,
      vline{Z} = {fg=\frameColor, wd=2pt},%last column
      hline{Z} = {fg=\frameColor, wd=2pt},%last row
}
  #7
  \end{tblr}
}{}
...
%%Better spacing		
	...
	References&{You references should be accurate and complete in relation to citations in the body of your dissertation}&\Cref{ssect:Processing}\\
