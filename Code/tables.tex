...
\hook_gput_code:nnn { begindocument } { tblr }
  {
    \IfPackageLoadedTF {changepage}
      {
%      	\write16{"Changepage~loaded:~setting~tblr~option"}
        \cs_gset_protected:Npn \__tblr_check_odd_page:
          {
          	\checkoddpage
%            \ifoddpage{\write16{"tblr:~oddpage"}}
%            \else{\write16{"tblr:~evenpage"}}
%            \fi
          }
...
\keys_if_exist:nnTF { tblr/__outer } { halign }
  { \write16{"halign~exists~in~tblr/__outer"}
  	\keys_define:nn { tblr/__outer} }
  { \write16{"halign~doesn't~exist~in~tblr/__outer"}
...
\newcommand{\narrowtablewidth}{\textwidth} %Ensure - isn't –
