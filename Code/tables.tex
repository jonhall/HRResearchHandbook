...
\NewTblrColumnType{R}[1][]{Q[co=1,#1,l]}%%Same as X

\let\narrowtablewidth=\textwidth
\let\widetablewidth=\textwidth

%%Simple N column table
\NewDocumentEnvironment{SimpleNColTable}{
		s					% * = not-long table, for tblrs in tcboxes
		m% 					% label
		m%					% number of columns
		O{\linewidth} 	% tablewidth (default: \widetablewidth)
		m% 					% caption
		O{X[1,l]*{10}{X[7,l]}}%	% optional colspec
		O{}					% Additional options
		+b%					% body
		}%
		{
		\IfBooleanTF{#1}{\begin{center}}	
			{\SetTblrOuter{	%%Add Outer only if non-star
				long,
	 			label={#2},
				caption={#5},
				expand=\headers,
...
%		\begin{tblr}
%			{
%	 			width = #4, % default = \linewidth
%				colspec = {#6},
%		  		hline{Z} = {fg = \frameColor, wd=2pt},
%				vline{Z} = {fg = \frameColor, wd=2pt}, %column{2} = {co=1}, 
%				colsep = 5pt,
%				column{1} = {fg = white, bg=\frameColor},%, font=\sffamily},
%				row{1} = {font=\bfseries, fg = white, bg=\frameColor},
%				rows = {m},
%				rowhead = 1,
%				measure=vbox, 
		...
		\begin{tblr}
			{
				...
				vline{1,2,Z} = {fg = \frameColor, wd=2pt}, %column{2} = {co=1}, 
				colsep = 5pt,
%				column{1} = {fg = white, bg=\frameColor},%, font=\sffamily},
				row{1} = {font=\bfseries, fg=white, bg=\frameColor},
				rows = {m},
			...
			#8
		\end{tblr}
	}{\IfBooleanT{#1}{%%If non-star version, ensure center
			\end{center}
		}
	}	
...
\NewTableCommand{\ResActTableHeader}{&\Gls{deliverable}&{Writing Outcome:\\ by the end of this stage you will}&Effort&Supervisor Interaction}
...
%Header					
	&\Gls{deliverable}&{Writing Outcome:\\ by the end of this stage you will}&Effort&Supervisor Interaction\\
%Body
		#2
	\end{tblr}}{}
	
%%All ReportTables follow this format
\NewTableCommand{\ReportTableHeader}{&\Gls{deliverable}&{Writing Outcome:\\ by the end of this stage you will}&Effort&Supervisor Interaction}
