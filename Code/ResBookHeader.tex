%%Git integration
%\usepackage[watermark]{gitstatus}

%%Bibliography
\usepackage[UKenglish]{babel}% recommended
%%Biblatex stuff
\usepackage[
%	backend=bibtex8,
	style=authoryear,
	autocite=inline,
	sortcites,
	backref=true,
%	backrefstyle=three,%consecutive pages are compressed
	refsegment=part,
]{biblatex}
\addbibresource{Bibs/MastersBib.bib}
\addbibresource{collatedBib.bib}

%%Xrefs
\usepackage{hyperref}% load after biblatex, from docs sect 1.5.4
\usepackage{xurl}

%%For fakeverb, so tables show \verb
\usepackage{codehigh}

%%Layout
\usepackage{geometry}
\geometry{
% showframe,
  left=2cm,
  right=1cm,
  top=1.5cm,
  bottom=1.5cm,
  marginparsep=8mm,
  marginparwidth=55mm,
  includemp,%pagewidth is done including marginpar
%  includehead,
  includefoot,
}

%%Fonts stuff
%%Explicit Italic, etc, for Texifier's old format
\usepackage{mathspec}
%%Hack from https://tex.stackexchange.com/questions/85696/what-causes-this-strange-interaction-between-glossaries-and-amsmath
\makeatletter % undo the wrong changes made by mathspec
\let\RequirePackage\original@RequirePackage
\let\usepackage\RequirePackage
\makeatother

\setmainfont[
	Mapping=tex-text,
%	BoldFont={* Bold},
%	ItalicFont={* Italic},
%	BoldItalicFont={* Bold Italic},
%%%Turns off ligatures
%	Ligatures=NoCommon,
%	Ligatures=NoRare,
]{Arial}
\setmathfont(Latin)[Scale=MatchLowercase]{BentonSans Book}

\usepackage{titling}
\usepackage[nameinlink,capitalise,noabbrev]{cleveref}
\usepackage{tocloft,calc}% for \widthof below

% headings and refs
\renewcommand{\partname}{Stage}
\crefname{part}{Stage}{Stages}
\Crefname{part}{Stage}{Stages}
%\providecommand{\partautorefname}{Stage}

% ToC: show "Stage I"
\renewcommand{\cftpartpresnum}{Stage~}
\renewcommand{\cftpartaftersnum}{\quad}
\setlength{\cftpartnumwidth}{\widthof{Stage~\Roman{part}\quad}}


%%%JGH 2024-10-03: fiddle with table of contents spacing
%\usepackage{tocloft}
%%\makeatletter
%%\renewcommand{\@pnumwidth}{2em}
%%\makeatother
%\setlength{\cftpartnumwidth}{4em}
%\setlength{\cftsectionnumwidth}{4em}
%\setlength{\cftsubsectionnumwidth}{4em}
%%\setlength{\cftnumwidth}{1cm}
%%%Add Part to TOC, response to JW335,JW166
%\renewcommand{\partname}{Stage}
%\renewcommand\thepart{\partname~\Roman{part}}

%%Colors
\usepackage[svgnames]{xcolor}

%%Footnotes
\newcommand{\footnotetextbulletsize}{1.8}
\usepackage[perpage,side,marginal,symbol*]{footmisc}
\DefineFNsymbols{hallrapanotti}{{\color{red}\scaletextbullet{\footnotetextbulletsize}}%
	{\color{green}\scaletextbullet{\footnotetextbulletsize}}%
	{\color{blue}\scaletextbullet{\footnotetextbulletsize}}%
	{\color{orange}\scaletextbullet{\footnotetextbulletsize}}%
	{\color{gray}\scaletextbullet{\footnotetextbulletsize}}
	{\color{purple}\scaletextbullet{\footnotetextbulletsize}}
	{\color{black}\scaletextbullet{\footnotetextbulletsize}}}
\setfnsymbol{hallrapanotti}

%%Set numbering depth
\setsecnumdepth{subsubsection}

%%Define the fullcites to be an itemized list
\newcommand{\fullcites}[1]{\def\multicitedelim{\item }\begin{itemize}\item \itemcites{#1}\end{itemize}}
\DeclareMultiCiteCommand{\itemcites}{\fullcite}{}
%%Autoinclude bib below (after Glossary)

%%Glossary
\usepackage[
	record,				% create group field and other stuff
	postpunc={dot},		% full stop after description
	nostyles,			% don't load default style packages
	stylemods={tree}	% load glossary-tree.sty and patch styles
]{glossaries-extra}

\GlsXtrLoadResources[
	src={glossary.bib},		% bib file
	selection={all},		% select all entries
	field-aliases={fulllong=long}, %%Glossary-extra manual, pg. 572
	trigger-type=ignored, 
	category={same as entry}
]
%%%To process the glossary, use bib2gls @MastersHandbook.aux in the appropriate directory

%%Quotes and such
\usepackage[threshold=1]{csquotes} %threshold in lines for blockquotes
%%Uncomment for italicised quotes
\renewcommand{\mktextquote}[6]{#1\emph{#2}#4#3#6#5}
\renewcommand{\mkblockquote}[4]{#1\emph{#2}#4#3}

%%Graphics
\usepackage{graphicx}
\graphicspath{{Figures/}}

%%Symbols
\usepackage{amssymb}

%%Numbering options to subsection
\setcounter{secnumdepth}{3}
\setcounter{tocdepth}{2}

%%Activity environment definitions
\input{environments.tex}
	
%%Table definitions
\input{tables.tex}
\input{WOtables.tex}

%%T.o.d.o notes
\usepackage[textsize=tiny,obeyFinal]{todonotes}

%%Load lasts
\usepackage{cleveref}
\usepackage{listliketab}