...
%}
%
%%Not used, other than in commented text
...
\cs_new_protected:Npn \readingextras:nn #1 #2
 {
  % Heading / intro, tweak to taste:
  \begin{question}[subtitle={Activity:~Deep~dive~into~#1}]%
  To~find~out~more~about~#1,~take~a~look~at~these~papers:%
  \begin{itemize}
    % #2 is a comma-separated list of keys
    \clist_map_inline:nn { #2 }
      {
        \item \fullcite{##1}
        % or, for "Author Year, Title":
        % \item \textcite{##1}, \mkbibemph{\citetitle{##1}}
      }
  \end{itemize}
  \end{question}
