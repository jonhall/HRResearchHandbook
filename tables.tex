...
\NewDocumentEnvironment{ReportTable}{
	...
	\begin{tblr}[long,caption={#2},label={#1}]{measure=vbox, stretch=-1,%%Allows lists in body, with no padding
					...
					row{1} = {fg = white, bg=\frameColor},%, font=\bfseries\sffamily},
					rowhead = 1,
					colspec={X[8,l]X[18,preto={\begin{itemize}[nosep,left=0pt]\item },appto={\end{itemize}}]X[4,preto={\raggedright}]},
					}
%Header					
...
\newcommand\ReportProgressTracking[7]%
	{\ReportLineHeader\titem{#1}{Appendix - Progress tracking}
	\begin{reportenum}
		\ritem{#2}{Fit}
		\ritem{#3}{Technical skills and resources needed}
		\ritem{#4}{Feasibility}
		\ritem{#5}{Further reading to be completed}
		\ritem{#6}{Work schedule}
		\ritem{#7}{Risk assessment}
	\end{reportenum}
}
