...
You've now reached Stage 4, which means the end of your project is in sight. In this stage you are in the midst of your data gathering and analysis, which is possibly the most exciting, yet demanding, part of your research: this is where you get an opportunity to make your original contribution to knowledge.

This stage assumes that you have gathered a certain amount of data and evidence, so that you can proceed with some analysis\footnote{If that's not the case, then, you should go back to Stage 3, Section~\ref{sect:???}, focusing on data collection. You should also discuss your progress with your supervisor, revisiting your project risk around access to the required data and evidence.}.
...
\section{Organising and storing your raw data}
Your \textit{raw data} represent anything you have gathered, say, full transcripts of participants' interviews, survey responses, logs of experimental data, copies of documents, etc. Before proceeding with your data analysis, you should ensure they are properly organised and stored to ensure you don't loose track of important information, and that you can always refer back to them during your analysis and beyond. You may also need to include such a sample of it as an appendix of your dissertation as demonstration of the data collection you have completed.
...
As indicated in Section~\ref{sect:dataAnalysisMethods}, common methods for qualitative data analysis include content, thematic, narrative or discourse analysis. For all of them, the first essential step is coding, which we discuss in this section.
...
Finally, whichever codes you end up with, you should ensure they are properly defined, so that their are unambiguous and can be applied consistently. You can use a \textit{codebook} for this purpose, which lists all the codes and their intended meaning, and that you can revisit and refine throughout the coding process.
...
From codes and categories, you can then proceed with your chosen analysis method, whether content, thematic, narrative, discourse analysis or other, in order to identify patterns and themes and provide your own interpretation of the data.


Although there are specialised tools for coding text\footnote{And even AI-based one which will do some initial coding for you...}, you can simply used your word processor, by using colours or comments to code segments of text. 

\begin{question}[subtitle={Activity: Using your word processor to code text}] 
*** give a text and ask to perform:
1) perform a 2-pass of inductive coding; write down the resulting codes
2) perform a 2-pass deductive coding, using codes: .... 
\begin{solution}
****
\end{solution}
\end{question}
...
While quantitative data can be summarised and presented using tables and charts, the same does not necessarily apply to qualitative data, which given their heterogeneous nature cannot be easily set out in a standard manner. 
...
\begin{itemize}
	...
	\item word clouds, to summarise emerging themes and concepts, and their relative frequencies
...
***LR-note: this may need to go in data collection

Sampling is the process of selecting a sample from the population of interest, and is relevant both in quantitative and qualitative research. 

Broadly speaking, sampling is classified into random\footnote{Also known as \textit{probability} sampling.} and non random. In random sampling an algorithm is established to extract the sample from the population, while in non-random sampling, the choice is based on the judgement and discretion of the researcher. The former is used when the ability to generalise to the population is paramount, while the latter when the depth and richness of the sample is more important. As a result, the former is used more in quantitative research, and the latter in qualitative research. Also, in quantitative research the tendency is to collect decide and collect the sample upfront, before any analysis commences, while in qualitative research, the process is iterative, with more and more data collected and analysed until no more collection is possible or \textit{saturation} is reached, that is collecting more data is unlikely to add more relevant information.
...
\begin{itemize}
	...
	\item snowball sampling, which relies on referral from previous participants to recruit new ones. This is an effective approach when a population is difficult to access or when the topic is sensitive or tabu. This too is unlikely to generate a representative sample, and it highly prone to bias.
\end{itemize}

Another important consideration is sample size,  which is particularly important in inferential statistics. As observed in Section~\ref{sect:inferentialStatistics}, in this context, if your sample is very large, almost all effects observed in the sample will be likely present in the population, where they may or may not be meaningful; vice-versa, if your sample is very small, most effects observed in the sample are unlikely to be present in the population, unless they are really large effect. As a rule of thumb, most tests require a sample size of at least 30 observations, but more precise sample size estimates can be made based on population size and expected significance level\footnote{Formulae for the ideal sample size are easily found in the literature and online.}

In summary, when choosing a sample, you need to consider various factors, including the aim of your study, the kind of methods you are applying, and the level of access you may have. Trade-offs are likely involved and you may not be able to obtain an ideal sample. Nevertheless, your sample will still be useful to your research, as long as you clearly explain and justify how it was obtained and what its limitation are.
