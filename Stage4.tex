...
\ResGenExtras{focus groups}{{powell1996focus}{smithson2000using}{plummer2008focus}}
...
You'll imagine that records – primarily written (or typed), but also photographic or video, sound, etc – of fieldwork constitute the main raw data gathered. These records – field notes – can have a number of forms\todo{From wikipedia, \parencite{enwiki:1211911661}: better source needed?}:
%
\begin{description}
...
\item [Journals and Diaries]	These notes record the ethnographer's personal reactions, frustrations, and assessments of life and work in the field.
\end{description}
%
\todo{More here}
...
If you don't have experience with programming languages, then you can learn, but the learning curve can be very steep\footnote{This video will give you some idea of what will be required: \url{https://www.youtube.com/watch?v=fLMZAHyrpyo}. Stephen Wolfram is a particularly interesting, if restless, speaker, and a leader in this area.}\todo{But this isn't a great video, nor is any other that I've looked at, as they're created by computer scientists... They're also heavily advertise-ful.}. 
...
Differential equations; Matrices; algebra, complex and other numbers; sets; logic; \todo{More here}

[Stolen from \cite{burton1984mathematical}:
Four processes can be shown to be central to mathematical activity and yet, Mathematical Thinking as before, to have general application. The four  processes are (a) Specializing,  (b) conjecturing, (c) generalizing, and (d) convincing. 
%
\begin{description}
\item [Specialising]When one is faced with a problem, a powerful way to explore its meaning is by examining particular examples. Such specializing is the key to an inductive approach to learning and is observed as natural to the learning of children. Each example provides the opportunity for manipulating elements that are concrete in the child's thinking, whether they are physical manifestations or ideas.
\item [Conjecturing] When enough such examples have been examined, conjecturing about the relationship that connects them happens almost automatically. Through conjecturing, a sense of any underlying pattern is explored, expressed, and then substantiated.
\item [Generalising] The recognition of pattern or regularity provokes the statement of a generalization. Such statements appear to be the building blocks used by learners to create order and meaning out of an overwhelming quantity of sense data, and it is on such generalizations that much behavior depends.
\item [Convincing.] To become robust, a generalization must be tested until it is convincing. first the thinker convinces himself or herself and then the world outside. The convincing process is the means by which a generalization moves from being personal to being public. A picture of the deductive approach is obtained by inverting the order of the processes. Beginning with a generalization, one explores the web of conjectures it provokes and tests them against particular specializations.
\end{description}
[end STOLEN]
...
%
\begin{itemize}
\item a particular area of research has already been taken past the abilities of a novice mathematician;
