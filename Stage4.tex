...
This section provides an introduction to the most commonly applied data analysis methods: it is far from complete and does not go very deeply into the details of each method. Instead, its purpose is to help you make your initial selection, after which you should review the related specialised literature to help you apply your chosen methods appropriately within your project. Your tutor's guidance will also be essential to this.

\subsection{Summarising data with tables}
In your research you may collect or make use of large amounts of raw data, possibly stored in a spreadsheet or database, so that a first step is to decide how to process such data in order to facilitate your analysis and to provide summaries for your dissertation.

To this end, tables are versatile tools used extensively in research and often found in dissertations. We will look at some key types in this section.

\paragraph{Pivot tables}

Pivot tables are used to summarise, sort, reorganise or group raw data from a spreadsheet, and also perform calculations on them such as counting or generating totals. As such, they are very powerful and versatile. 

For instance, assume we have collected data concerning all students who have studied a particular module over a period of time. For each student, we have both demographic data, such as age, gender, ethnicity, etc. and study data, such as study co-hort, outcome and degree studied. These make up several hundred entries in a spreadsheet, which makes it difficult to analyse them and identify patterns of interest. Pivot tables can help, for instance, by allowing us to summarise the data in many different ways which can help us answer questions we may have. For instance, we may like to know how the outcomes differ between co-horts, in which case, we may a pivot table where **** change example so that we can have both counts and something else!!!! 


Activity - investigate pivot tables in Excel 


\paragraph{Frequency tables}
Frequency tables are used to summarise the frequency (or count) of values taken by a categorical variables in a data set. For instance, after studying a degree, a student's outcome may be classed as distinction, merit, pass or fail. A frequency table can then be used to summarise the frequency of each class of outcome for a particular students' cohort, as shown in Table~\ref{tab:frequencyTableExample}. Frequency tables can, of course be created as pivot tables.
...
\paragraph{Contingency tables} Contingency tables\footnote{Also known as \textit{cross-tabulation} tables.} are a form of frequency tables used to tabulate the value frequencies of two categorical variables. For instance, following from our previous example, we may like to tabulate the outcome value frequencies in the cohort against gender, as shown in Table~\ref{tab:contingencyTableExample}. Contingency tables too can also be created as pivot tables.
...
***to decide how far to go -- rabbit hole!
...
These are lots of definitions to digest, particularly if you haven't these terms before! The following activity should help.
...
\begin{guidance}
MS Excel is relatively straightforward to use for this purpose, but feel free to use other tools  you may be familiar with, including statistical or data analytics packages. If you do, you should ensure they support the functions we have discussion in this section.
\end{guidance}
\end{question}


\subsubsection{Inferential statistics}
Inferential statistics relies on the concepts of population and sample: the \textit{population} is the entire group you are interested in studying -- say, all UK voters in a general election; while the \textit{sample} is the portion or subset of that group you have access to in your research. Then the aim of inferential statistics is to establish whether patterns you have observed in your sample can be generalised to, i.e., inferred for, the whole population and possibly used to make predictions about that population, or whether they are the result of chance. For this, it is essential that the sample is representative of the population, something we will consider in some detail in Section~\ref{sect:sampling}.

In inferential statistics this is achieved by testing hypotheses, where an \textit{hypothesis} is an proposition you wish to test -- in essence, an educated guess you have come up with based on some observations you have made, but has yet to proved. 
...
\begin{itemize}
	\item subjective sampling, the researcher chooses based on the researcher's own judgement and research aim. Used for small populations or especially rare population; target individuals with particular knowledge or expertise; prone to bias and unlikely to lead to generalisable results. Better suite for narrow and deep
