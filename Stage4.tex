...
This stage assumes that you you have worked out most of your research design details and are now in a position to make good progress with you data collection and analysis\footnote{If that's not the case, then, you should go back to Stage 3. You should also discuss your progress with your supervisor, revisiting your project timescale and risk.}.
...
Your \textit{raw data} represent any data and evidence you gather as part of your research. Which data you gather and how is determined by the choices you have made in your research design, informed by your research aim and objectives. In this section, we touch on key topics in data gathering which are relevant to many projects, although they may not all apply to yours. This section is only meant as an introduction to those topics, to raise your awareness and point you towards specialised literature, should you need it, to deepen your understanding.

\subsection{Sampling}\label{sect:sampling}
Sampling is the process of selecting a sample from the population of interest, and is used in many  research strategies, for instance survey (who will fill in your questionnaire) or experimental (who will take part in your experiment) research. 

Broadly speaking, sampling can be random\footnote{Also known as \textit{probability} sampling.} or non random. In random sampling an algorithm is used to extract the sample from the population, while in non-random sampling, the choice is based on the researcher's judgement and discretion. The former is used when the ability to generalise to the population is paramount, while the latter when the depth and richness of the sample is more important. As a result, the former is used more in quantitative research, and the latter in qualitative research. Also, in quantitative research the tendency is choose the sample upfront, before any analysis commences, while in qualitative research, the process is iterative, with more and more sample data collected and analysed until no more collection is possible or \textit{saturation} is reached, that is collecting more data would not bring more relevant information.
...
\begin{itemize}
	...
	\item stratified random sampling, where sub-groups of the population are identified based on common characteristics, the \textit{strata}, and sampling is random across those strata. The strata are not mutually exclusive: for instance, the population may have sub-groups defined by gender, ethnicity and level of education, which may overlap. This approach overcomes the over/under representation problem of simple random sampling.
	\item cluster sampling, where the population is divide up in naturally occurring separate clusters, and the sample is obtained by randomly selecting some clusters and then randomly selecting members of those clusters. It is more cost-efficient than the other two approaches, but can introduce bias if the selected clusters are not representative of the whole population, so that the over/under representation problem remains.
\end{itemize}

Non random sampling includes:
\begin{itemize}
	\item purposive sampling, in which participants are selected by the researcher based on particular characteristics, knowledge or expertise they may have. It is often used for small populations, especially rare population which are difficult to access. It is particularly suited to studies which intend to be deep and narrow, and for which generalisation is not a concern. As the sample choice is made by the researcher, it is prone to bias.
	...
	\item snowball sampling, which relies on referral from previous participants to recruit new ones. This is an effective approach when a population is difficult to access or when the topic is sensitive or tabu. This too is unlikely to generate a representative sample, and is prone to bias.
\end{itemize}
...
\paragraph{Double-barreled questions} Also termed `compound', these are questions that ask more than one thing, while only allowing one answer. These should be avoided as it would be difficult, if not impossible, to establish in your analysis which part of the question each participant has answered. Instead, you should split the question into separate questions each addressing a specific thing.

\paragraph{Response options} Questions are broadly divided into closed and open-ended. The former restrict the possible responses to a set of given choices, while the latter allow respondents to use their own words freely to answer the question. If you use closed questions you need to ensure that the possible answers are exhaustive, in that they cover all possible options, and exclusive, in the sense they don't overlap. Open questions can lead to richer answers, but you must ensure they are sufficiently constrained so that the answers don't end up being too vague or off topic. 
...
\paragraph{Question grouping, ordering and flow} Related questions, say, all demographic questions, should be grouped together, and the flow between groups of questions should be logical. The questions order in each group also matters: as a rule of thumb, simpler questions should precede more complex ones.

\paragraph{Bespoke tools} While you can design your questionnaires from scratch using your word processor, there are plenty of specialised digital tools, many of which are free, that can make it a lot easier for you to design your questionnaires. They usually come with: templates and pre-defined question types that you can customise for your study; statistical analysis and data visualisation features that you can apply to the data you have collected; export functions that  allow you to save the data to a spreadsheet for further analysis. Overall, if you need to develop questionnaires for your research, they can really help you speed up the process, so that they are well worth considering. 

\paragraph{Testing your questionnaire} Once you have a first draft of your questionnaire, you should test it and refine it before you deploy it in your data collection. This will help you identify issues in relation to any of the points we have just discussed, and avoid wasting time and resources by employing a poorly design questionnaire in your study. Testing can be done initially by simply asking a friend, family member or colleague to go through it and provide some feedback, or ask an expert to review it, including, of course, your supervisor. In addition, you could pilot your questionnaire on a small number of respondents first, then revise it as necessary before using it more widely. In all cases, you are looking for evidence where confusion (which may point to lack of clarity), or hesitation (which may point to poor response options or inappropriate scales) or disengagement (which may point to too many questions) may have occurred. 
...
In writing up your data analysis in your stage report or disseration, you will need to decide:

\begin{itemize}
	\item how to summarise data and evidence. This will depend on the nature of your data and evidence, and you will need to ensure that your summaries are appropriate to convey the essence of the evidence you have generated. In the previous sections, you have considered ways in which quantitative and qualitative data can be summarised using tables and visualisations. If necessary, sample raw data should be included in an appendix.
...
By interpretation of your findings, we mean addressing the following questions:
