...
With reference to our 5-stage framework, the activities which are in focus in Stage 4 are summarised in~\Cref{tab:stage4}, which also provides some guidance for your interaction with your supervisor during this stage.

\subsection{Your Research Activities for this stage}

\todo{More here}

\begin{SimpleThreeColTable}[\RActivitiesTableCaption{1}\label{stage1ResearchActivities}][X[1]X[2]X[2]]
...
\begin{question}[subtitle={Activity: Understanding the effort needed in this stage}] Consider~\Cref{tab:stage4} carefully, paying particular attention to the entries in the `Effort' column. Make a note of the activities which are most prominent in this stage and what their deliverables and learning outcomes are.
...
Modern standards of research also require you to comply with regulations on data privacy and protection\footnote{We discussed GDPR in~\Cref{sect:personalData}.}. These put many constraints on the use of personal data as they are designed to protect the identity of the people they refer to. 
...
\subsection{Procedural considerations}\label{ssect:ProceduralConsiderations}
...
\subsection{Further reading}\label{ssect:FurtherReading}
...
\subsection{Operational considerations}\label{ssect:OperationalConsiderations}
...
\paragraph{Data gathering} To gather your data about the system you can apply any relevant method from~\Cref{sect:genMethods}. It is likely you will need both qualitative and quantitative data as you are aiming for a comprehensive characterisation and analysis of your system from a plurality of perspectives.
...
\ResGenExtras{systemic thinking}{{??}}
...
\begin{SimpleFiveColTable}[Example of frequency table\label{tab:frequencyTableExample}]
...
\begin{SimpleFiveColTable}[Example of contingency table\label{tab:contingencyTableExample}]
...
\begin{SimpleTwoColTable}[Values][{rS[table-format=3.2]}]
...
\begin{SimpleThreeColTable}[Common charts to visualise data sets and their descriptive statistics\label{tab:charts}]
...
\begin{SimpleSixColTable}[Common statistical tests for comparison. \textit{Parametric} tests apply to normally distributed data (see Section\Cref{sect:descriptive}), while \textit{non parametric} tests to skewed distributions.
...
\end{SimpleSixColTable}

\begin{SimpleSixColTable}[Common statistical tests for association. \textit{Parametric} tests apply to normally distributed data, while \textit{non parametric} tests to skewed distributions.
	\label{tab:testsForAssociation}][X[8]X[2]X[8]X[1]X[1]X[8]]
	Purpose & Variables & Example & Parametric & Non parametric & Notes  \\
...
\chapter{Managing risk in Stage 4}\label{ch:ManagingRiskIn}
\todo{More here}

\chapter{Reflecting and reporting in Stage 4}\label{ch:ReflectingAndReporting}
...
\begin{SimpleTwoColTable}[Report structure and guidance \label{tab:S4report}]
...
\begin{SimpleTwoColTable}[Criteria for reviewing your research proposal \label{tab:criteriaForStage4report}]
...
\item Questionnaires are common tools for data generations. Good questionnaire design relies on a wide range of considerations (see~\Cref{sect:questionnaireDesign}).
