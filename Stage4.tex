...
So far we have discussed methods which focus on generating data from either direct observations or experience (whether the researcher's or other research participants') in relation to natural or social phenomena, or data from secondary sources. Once gathered, such data are then organised and analysed by applying data analysis methods, which we will consider in the next section.
...
Statistical thinking is problem solving with Statistics. It is used to identify patterns, trends and relationships in data from which inferences are possible through probabilistic reasoning, which acknowledges the data inherent variability and uncertainty. Such inferences are particularly valuable for prediction and forecasting -- for instance, to predict the spread of a virus in a population or how the average house price may change over a certain period of time, or to test hypotheses\footnote{We will introduce statistical testing in Section ??}, for instance whether a medical treatment is effective.
...
\paragraph{Sampling} Statistical modelling requires relevant data, so that you have to considered how you will obtain such data. Sampling is a way to do so\footnote{You should refer back to Section??}: in such case, you need to worry about both sample size and the extent it is representative of the population of interest.

\paragraph{Data quality} Quality data are data with no missing, inconsistent or erroneous entries. Typically, you will need to pre-process your data to ensure that is the case, before applying any statistical technique. 
...
\paragraph{Statistical software} The use of computational tools is the norm in statistical thinking and many bespoke statistical software applications are available on the market. In addition, many programming languages and environments come now equipped with full libraries of statistical techniques ready for use\footnote{R and Python are two of them.}. Such applications and environments allow you to perform data manipulation and visualisation alongside most statistical modelling and testing.

\subsubsection{Other things to think about}


\paragraph{Foundational knowledge} Statistical techniques can be complex and require specialised knowledge and skills to apply them effectively, particularly for advanced modelling, but also to provide meaningful interpretation of outcomes. As for mathematical thinking, it is unlikely you will be able to develop knowledge and skills beyond the basics within the remit of Masters research.

\paragraph{Data bias} Even when your data are of good quality, they can still be biased in that they may over- or under-represent certain characteristics of the population of interest, leading to invalid or unreliable generalisations. For instance, if all participants in a clinical trial for a new medicinal drug are male, then the effectiveness of the drug for other genders is not tested by the trial, so that any generalisation to the wider population may be unsound.

\paragraph{Confounding factors} Unfortunately, things can still go wrong even when you have good quality, unbiased data to start with. This may be due to \emph{confounding factors} you have not considered in your study. For instance, say you are interested in the possible relationship between physical activity and heart health. If you only focus on those two variables, you are likely to miss possible effects of, say, age and gender on heart health in addition to physical activity. 

\paragraph{Model assumptions} Statistical methods are usually based on specific assumptions made of characteristics of the data. For instance, many statistical tests assume that the data are normally distributed\footnote{We will cover these topics in Section ??}. It is therefore essential for you to check all required assumptions hold, otherwise your model, and any conclusions you derive from it, may be invalid.

\paragraph{Ethical considerations} As statistical thinking relies on data, then ethical and legal issues may arise in relation to how the data are obtained and used in research, but also in terms of the social implications of applying statistical thinking in decision-making, particularly when decisions are increasingly taken by algorithms. The former relate to privacy and data protection, while the latter to potential inequality and discrimination\footnote{Some shocking examples have been reported widely, e.g., the COMPAS system, discriminating against black offenders, or the Amazon's recruitment algorithm, discriminating against women.}. 
