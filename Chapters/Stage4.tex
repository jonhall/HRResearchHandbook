...
All research needs data and this chapter steps through the most used data generation methods. Most of them concern {empirical data}, that is data that is obtained through our five senses or from experience, yours or that of your study participants. 

 The term \gls{data-generation} doesn't necessarily mean that you will create a new-to-the-world \enquote{data}\footnote{Although this might indeed be the outcome of your data generation.}. It simply means new-to-your-project data, which covers a multitude of sins, including:
...
\begin{itemize}
\item having two moderators with separate roles, say one looking after the procedures, the other focusing on the discussion, or both contributing to the discussion, but taking opposite sides or with one playing devil's advocate\footnote{The term `duelling moderators' is used in this case.}
...
\item a `mini' focus, with a reduced number of participants to create a more intimate atmosphere for the discussion.
\end{itemize}
...
Fieldwork is a data generation method which requires the researcher to collect data directly in a natural setting\footnote{The `field', hence the name.}. The goal is to gain first-hand knowledge of the phenomena under study, and the method is widely applied across many disciplines, including anthropology, sociology, archaeology, geography or environmental science. 
...
\begin{SimpleNColTable}{tab:modellingMethodsChoice}{2}{Modelling methods in this chapter}[ll]
	Method & Section\\
	\Gls{computational-thinking}&\Cref{sect:ComputationalThinking}\\
	Mathematical thinking&\ref{sect:MathematicalThinking}\\
	Statistical thinking&\ef{sect:StatisticalThinking}\\
	Systemic thinking&\ref{sect:SystemicThinking}\\
\end{SimpleNColTable}
...
Apply your chosen visualisation techniques\todo{gloss} to your qualitative data.
\begin{guidance}
...
There are two broad categories of statistical methods\todo{gloss}:
\begin{itemize}
...
\begin{itemize}
	...
	\item centrality, to establish the `centre' of the data set. Three measures are commonly used: the \gls{mean}, which provides the average value of the data set; the \gls{median}, which provides its mid point\footnote{Remember that quantitative data can be ordered.}; and the \gls{mode}, which indicates the value that occurs most frequently, if any\footnote{There is no mode if no value is repeated in the data set.} 
...
\begin{guidance}
MS Excel\todo{gloss} is relatively straightforward to use for this purpose, but feel free to use other tools you may be already familiar with, including statistical or data analytics packages. Whichever tool you use, you should ensure it supports the functionalities discussed in this section.
\end{guidance}
\end{question}
%%Hack to correct tcbox behaviour
\color{black}
...
You use a statistical test to find out whether a \gls{proposition} is likely to be true in the population you are studying. You can think of a proposition as an educated guess you have made based on some observations, but that has yet to be supported by evidence. To test a proposition, you need a representative sample of the population\footnote{Sampling\todo{gloss} was discussed in~\Cref{ssect:sampling}}.
...
\end{guidance}
\begin{solution}
Assuming normal distributions\todo{gloss}, for each scenario, you should consider:
\begin{itemize}
...
You can choose your codes in various ways. For instance, \gls{in-vivo} coding uses the exact language which occurs in the data: this is used, in particular, for participants' speech, especially when different languages are used. Instead, \gls{descriptive} coding\footnote{This is a very common approach, although there are others which you can research in the literature.} uses words which encapsulate a general idea, such as `sport' or `running': this is particularly useful for non textual data, like images or videos. 
