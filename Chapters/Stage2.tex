...
\begin{solution}
The Open University has no single format for research dissertation. However, one specific \gls{capstone-project} dissertation guidelines include:
%
\begin{enumerate}%[start=0,label={(\bfseries R\arabic*):}]
...
\begin{solution}
The Open University has a strict policy on \gls{plagiarism}. 
...
The development and presentation of academic arguments forms the core of academic writing. But what exactly is an academic argument? To answer this question, this book used the \gls{model} proposed by Booth, Colomb and Williams in \textcite{booth1995making} -- referred to as the \gls{bcw-model} (BCW for short). 

\subsection{The BCW model}\label{ssect:BCWModel}

In the BCW model, an academic argument has at least two parts – the \textit{\gls{claim}} and the \textit{\gls{evidence}} – and might, in more complex arguments, have up to three other parts – called the \textit{\gls{reason}}, the \textit{\gls{warrant}} and any \textit{qualifications}. 

The \gls{model} is shown in~\Cref{fig:arguments}, with its parts:
%
%
...
\item ...the \textit{\gls{reason}}, which is why you believe the \gls{claim} to be true. In addition ...

\item the \textit{\gls{warrant}} explains how the \textit{\gls{reason}} is relevant to the \textit{\gls{claim}}, while ...

\item the \textit{\gls{qualifications}} are concessions which may limit what is being claimed, for instance by acknowledging objections, alternatives,~\etc{}.
\end{enumerate}
%
...
\begin{question}[subtitle={Activity: Constructing your academic arguments}] Consider your current thematic summaries. Extract and write down their academic arguments according the BCW \gls{model}. Identify those which are already well developed and those which require further work, indicating what is still needed.
...
\begin{itemize}
	...
	\item \Gls{false-dichotomy}, in which an \emph{either--or} \gls{claim} is made which assumes that there are two mutually exclusive \glspl{phenomenon} where that's not the case --- the \glspl{phenomenon} may not be mutually exclusive at all. 
\end{itemize}
...
\begin{itemize}
	...
	\item \Gls{post-hoc-ergo-propter-hoc}: Italy used to loose in the rugby Six Nations tournament till I started to watch them play. Therefore, my watching is causing them to win (assumed false causal relation)
...
Your thematic summaries are your starting point to populate the main body of your literature review. In doing so, you should take the following steps\footnote{You should have both your thematic summaries and \gls{concept-map} (see~\Cref{ssect:Synthesising}) at hand.}:
...
\begin{question}[subtitle={Activity: Writing the introduction}] Write a brief introduction to your literature review to introduce the topics you are going to cover in its main body, and their relevance to your \gls{research-problem}.
\begin{guidance} 
...
\begin{description}
\item [\Gls{ontology}] is the philosophical study of the nature of existence and addresses the question: \enquote{What is the reality that I will research?}. Practically, \gls{ontology} translates to determining which \emph{\glspl{phenomenon}} exist in the context of  your research, the \emph{relations} that exist between them and how they group together into \textit{categories}.
...
\item [\Gls{research-methodology}] is the \gls{system} of principles and methods by which you conduct research, that is, investigate \glspl{phenomenon}, and generate and analyse data and \gls{evidence}. Methodology operationalises the \enquote{how} question of knowledge generation, so it is about devising concrete ways to answer \enquote{How will I make my contribution to knowledge?}. 
\end{description}
...
The \glspl{phenomenon} upon which your research will be based must be observable and this gives rise to data. \Glspl{datum} are the raw observations with no interpretation attached --- anything you may collect, capture or gather in your research. \Gls{evidence} is information interpreted to support (or otherwise!) your academic arguments. Indeed, data forms the basis of \gls{evidence}, so the two concepts are closely linked and often used interchangeably. This section recalls briefly the main types of data and \gls{evidence} used in \gls{academic-research}.
...
The \gls{academic-literature} at the core of your literature review is \gls{secondary-evidence}, as are all other published academic and non academic documents, e.g., laws, policies and procedures, official reports,~\etc{}.
...
The quality of \gls{critical-theory} research is judged in terms of how well it is situated in its historical context, and the extent it acts as a stimulus for transformation, and for reducing ignorance and misconceptions.
...
It is important to note that characterising this paradigm in relation to \gls{ontology}, \gls{epistemology} and methodology is rejected by some scholars, seen as a form of colonialism imposed by a Western view of \gls{research-paradigms}\footnote{If you are interested in this debate, you could start from \cite{hart2010indigenous}.}.
...
%	
	...
	\begin{solution}
		...
		The first is to start by observing the swans that live on a lake near your home, and record your observations. Assuming they are all white, you could put forward an initial \gls{hypothesis}, say that all \enquote{swans are white}. You could then look online for images of swans from around the world to see if they match your observations. Having found images of black swans alongside white ones, you would then revise your \gls{hypothesis} to \enquote{All swans are either white or black.} This process could continue until you're satisfied there is no further contradictory \gls{evidence}, hence conclude that in all probability swans are either white or black. You would have to admit that there may be swans of other colours you've yet to come across, so the statement is open to future challenges. You would also need to be convinced that you are a neutral observer, able to determine the colour of a swan correctly and reliably. This approach closely aligns with the \gls{post-positivism}[post-positivist] paradigm, specifically: you make observations, triangulate them by reviewing online swan images, and formulate, rejecte and then reformulate \gls{hypotheses} as part of your enquiry process.
		
		A second approach would be to ask other people. For instance, you could set up a crowd-sourcing survey inviting participants to answer the question. By analysing their answers you could then decide if there is enough consensus on the colour of swans: for instance, most participants may have identified swans to be either white or black, although some may have provided more nuanced answers, like yellowish or other. From your analysis of the answers, you would draw your conclusions which may or may not be the same as in the previous approach. In this case, you would have to worry about who participated in your research. Were there enough participants from around the world to provide sufficient and diverse evidence? To which extent may their colour perception differ? Were their trustworthy in their answers? What else could you do to check the \gls{validity} of the outcome? This approach aligns with the anti-positivist paradigm: you have to accept that, each observer in your study will make their own interpretation of what the colour of a swan is, so that you would have to account for possible differences in your conclusions. 
\end{solution}
\end{question}
%%Hack to correct tcbox behaviour
\color{black}
...
\Gls{systemic-thinking} & problem solving approach focused on understanding and improving complex systems and their dynamics \\
\end{SimpleNColTable} 
...
A \gls{research-strategy}\footnote{A well-known take on {strategy} is that of the economist \textit{Michael Porter}: \enquote{The essence of strategy is choosing what not to do}. Your \gls{research-strategy} should do this – by giving you a focus – so choose carefully, and use its guidance wisely.} is a systematisation of a set of research methods, which can be applied together in order to address research problems of a particular kind. As such it can help you select and apply an appropriate mix of research methods to generate and analyse data, and derive insights. 
...
\Gls{ethnography} & in-situ, participatory study of cultures, including practices, meanings and interactions, reported as rich (or \enquote{thick}) description\\
...
Supporting \gls{evidence} & Are all your claims supported by appropriate citations or other \gls{evidence}? Which further \gls{evidence} do you still need?\\
