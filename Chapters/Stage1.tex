\MRpart{The Scope}{part:scope}{In all research, but particularly at Masters where time is limited, it is important to establish the scope of your project early on: this is the main focus of Stage 1.}
...
In this chapter, you will start by thinking about how you will organise your work in this stage, while in the following chapters, we will look at the other activities in turn.
...
The first column of the table gives you the expected full structure of the dissertation. Within this column, we have greyed out all parts which are yet to be written and highlighted in red those you will write from scratch during this stage. The guidance in the second column refers to the latter, with the third column indicating where to go in chapter and sections of the book for this stage to find relevant materials. 

The dissertation structure is a generic structure based on our experience as research supervisors and examiners. Your university course may provide a different template\footnote{Something you should check at this point!}, which, however, should still include all the parts we have in ours, perhaps in a different order or with different titles. You should ensure you understand the similarity and differences between our template and the one you will need to use for your studies, so that you can still make good use of~\Cref{stage1WritingOutcomes}.
...
\begin{guidance}You should make a note of key ideas, theories, approaches or principles from your course or its discipline, which are particularly relevant to the topics you have chosen. For thought degrees, identify specific study materials you may like to revisit or apply in your project.
\end{guidance}\end{question}
%%Hack to correct tcbox behaviour
\color{black}
...
the Intellectual Property\footnote{Intellectual property (IP) is property of things people create with their own intellect, such as an invention, an artwork, a design, etc. We will return to IP in~\Cref{sect:IntellectualProperty}.} that you create through your research may lead to solutions to their specific problems, and possibly new products and/or services that might generate a revenue stream that you can share.
...
For a research student's project, that larger research community might be represented by a very small group of people: your supervisor and your dissertation examiners. In some cases,\footnote{Many of our research students have done this.} a research student might go on to report their research findings to a larger community of scholars at an academic conference, for instance, or even through a scientific journal: conference or journal publication is the pinnacle of validation that knowledge is new.

\subsection{Problem formulation}\label{ssect:ProblemFormulation}

We can now write down Anna's problem using the template we have provided and based on what we have found out so far, which is summarised in~\Cref{tab:Annaproblemelements}.
...
In~\Cref{ch:ReviewingTheLiterature} we will look in detail at how you can do that, but first let's look more closely at different types of research problem.

\section{Types of research problems}\label{sect:problemTypes}

Research problems originating from our template depend critically on phenomena. And it won't come as any surprise then, that you can classify the types of research problem by the things you can do with those phenomena. And that leads to the different types of research that you can do.
...
\begin{question}[subtitle={Activity - Exploratory problems}] For each example problem above, identify its constituent parts with reference to the research problem template we have provided. Write down any similarity between the problems.
...
\begin{question}[subtitle={Activity - Explanatory problems}] For each example problem above, identify its constituent parts with reference to the research problem template we have provided. Write down any similarity between the problems.
...
\begin{question}[subtitle={Activity - Predictive problems}] For each example problem above, identify its constituent parts with reference to the research problem template we have provided. Write down any similarity between the problems.
...
\begin{enumerate}
	\item To evaluate the effectiveness of Artificial Intelligence (AI) to reduce the occurrence and impact of successful cyber-security attacks within the financial sector. This matters to the sector because over 3 billion dollars are lost within the sector every year due to successful attacks, and current methods to prevent them are also very costly.
...
\begin{question}[subtitle={Activity - Evaluative problems}] For each example problem above, identify its constituent parts with reference to the research problem template we have provided. Write down any similarity between the problems.
...
\begin{question}[subtitle={Activity - Design problems}] For each example problem above, identify its constituent parts with reference to the research problem template we have provided. Write down any similarity between the problems.
...
While more specific than your aim, your research objectives are still going to be relatively broad at this point, so you should not enumerate every little thing you are going to do! Usually 3 to 5 main research objectives are sufficient: later on you can break them down into several specific tasks, but we are not there yet. 
...
In this section, we'll be looking at how to read an academic paper. It may seem strange to think about trying %\footnote{Searching comes next, promise!} 
	to read an academic article without having found any first! It's such an important skill, however, that it needs introducing early, and in a way that we can coach you through it the first time. Honestly\footnote{And not just for your research project. Reading the \gls{academic-literature} is a professional skill that not many professionals have.}, it's going to take some time to pick up this skill, but it's an investment worth making. 
...
\item The second pass gives you a better grasp of the content in outline of a relevant paper. It shouldn't take you more than one hour, and gives you the opportunity to annotate the paper\footnote{We mean write your notes on it that capture your growing understanding.} as you begin to understand it. You can stop after the second pass if you need to --  perhaps you have a large pile of papers that you wish to sort for relevance.
...
Use Google Scholar to search the article title and then download the paper.
...
Reading Keshav's paper using Keshav's workflow will give you a head-start on the following,%\footnote{It's also satisfyingly recursive!} --- 
	something we've learned in the time Keshav's paper has been in use by our students. 
...
We recommend you apply Keshav's workflow when reading the \gls{academic-literature}. But first, let's consider the literature review process as a whole.
...
We can take each problem element in turn to identify possible search terms. In doing so, we have come up with this initial set, which should be sufficient to perform our initial search. The results are shown in~\Cref{tab:searchTerms}.


\end{running}

\begin{SimpleNColTable}{tab:searchTerms}{3}[\textwidth]{Brainstormed search term}
[R[3]R[6]R[6]]
...
\begin{enumerate}
	...
	to Google Scholar and this returned 5310 results: that's a lot of papers to read, so we knew we needed to narrow the search down.
	...
	\item To narrow down the search further, we typed in:
...
For each search, record both the combination you use and how many articles Google Scholar returns as hits. Reflect on how using different terms and operators may change the number of hits and how that may inform ways to widen or narrow down your future searches.
...
Although in this section we have focussed on Google Scholar, the same techniques apply to most bibliographical database search engines, so you could repeat the activity on any database you can access through your library.
...
to our last search term, and only considered publications since 2020. The resulting list included only 9 hits, of which 4 were literature reviews.
\end{solution}\end{question}
%%Hack to correct tcbox behaviour
\color{black}
...
If you use a BMT, which we recommend, you should use its facilities to enter and store such bibliographical information, and generate your reference list automatically for inclusion in your dissertation and in the style required by your course of study.
...
First thing you should do is to record it in your BMT. You should record it with the search term that you used to find it so that you can rerun the search to find:
...
\begin{itemize}
	...
	\item Methods: how the authors conducted their study. Don't worry if you don't understand all the details at this point: we will look at research methods in detail in Stages 3 and 4
...
%\end{itemize}
We conducted a literature search, out of which we identified a number of potentially relevant articles. We constructed a summary-comparison matrix, whose first two entries are illustrated in~\Cref{t:exampleSCMatrix}. You may notice that our entries are quite detailed, although it is not necessary for you to understand all the details. 
\end{example}
...
To this end, we recommend you highlight concepts of interest within your summary-comparison matrix, then collect them in a \gls{concept-matrix}. This is yet another table, with one concept per row and one article per column, and a tick at their intersection to indicate whether the concept appears in the article. Some concepts will recur across several articles, and the concept matrix is a useful visual tool to identify those recurring concepts. 
...
One of the entries in our example summary-comparison matrix reads:
\begin{quotation}
...
\end{quotation}
from which we can extract a couple of concepts:
\begin{itemize}
...
Carrying on with our example, we have identified a number of concepts in our summary-comparison matrix, from which we have constructed a concept matrix, a small extract of which is given in~\Cref{t:exampleCMatrix}.
...
\end{itemize}
We have, however, avoided details about research methods at this point, as our focus is to characterise and justify our research problem. Research methods are, of course, very important, and something we will consider in detail in Stages 3 and 4.
\end{example}
...
\begin{guidance}
You could start by applying the heuristics from our example to identify concepts in your summary-comparison matrix, but do not be constrained by them, and feel free to come up with other heuristics to guide your choices. Expect to go over each entry a number of times before reaching the set of concepts you wish to focus on.
\end{guidance}
\end{question}
%%Hack to correct tcbox behaviour
\color{black}
...
\begin{description}
	...
	\item[Step 3] iterate until each concept is included under one theme. This may require you to change your mind and move concepts around, or refine or create more themes.
\end{description}
%
Note that this is yet another iterative process, so don't expect to complete your concept map in one pass!

 Also note that the end of this process, some groups may be larger than others, in the sense that they may contain more concepts and from several articles: the group `size' provides an indication of which themes feature more prominently in the articles your have reviewed. 
...
%
%\begin{question}[subtitle={Activity: Considering your university ethical and legal guidelines}] Look up your university and course regulations to check any specific ethical and legal guidelines or constraints which apply to your project. Write a brief summary of what you have found out.
%
...
\begin{solution}
Our university has a very comprehensive set of ethical guidelines and codes for research involving human participants, covering expected behaviours and protocols, including the need for explicit approval from the university ethical research committee to conduct certain kinds of research, how to handle personal data and how to safeguard the health and safety of all participants.
\end{solution}\end{question}
%%Hack to correct tcbox behaviour
\color{black}
...
Within the European Union, the General Data Protection Regulation (GDPR) applies. %\footnote{At the time of writing{\ldots}} 
...
Even if you think GDPR will not apply to your research, we would still encourage you to apply its principles in your project, to guard against an improper use of personal data.
...
\begin{itemize}
\item \gls{equity} relates to fairness and justice, in the sense of removing barriers or bias\footnote{We discuss bias in~\Cref{sect:biasInResearch}.} which may prevent individuals, or groups of individuals, from having equality of access, opportunity or outcomes
...
\begin{solution} At our university, the IP policy establishes rights in relation to the ownership, development, protection and exploitation of IP arising from all types of research and scholarship carried out in the institution. For instance, for Masters research projects which are part of a taught course of studies, our university usually assigns the IP to the student. Instead, IP arising from doctoral research belongs to the university, unless special agreements are in place.
\end{solution}\end{question}
%%Hack to correct tcbox behaviour
\color{black}
...
\begin{question}[subtitle={Activity: Understanding your university guidelines on GenAI}] Using your university intranet look up any policy or guideline related to the use of generative AI. Write a brief summary of what you have found out which may be relevant to your research project.
\begin{solution} 
...
In our experience, both of these can hamper progress. The latter is especially pernicious: that you see a problem is great, but can you find sufficient evidence in the \gls{academic-literature}?
...
For your project, we recommend you use either a qualitative or a rating approach, as the whole point of assessing risk in a research project is to think deeply about it and be ready if things go wrong, rather than quantifying risk precisely.


\section{Research project risk}\label{sect:ResearchProjectRisk}
In your research project there will be risk which is very specific to what you intend to do, but there are also risk categories which are common to all projects, which we consider in this section.
...
\begin{guidance} You should pay particular attention to your university regulations in regard to those issues, in particular if your project is likely to involve human participants or  handling personal data, and to assess risk related to possible bias.
...
It is time to summarise the outcome of your risk analysis. To do so, you could use a risk table similar to~\Cref{tab:riskTable}. Note that the table encourages you to consider also risk specific to your project which may not fall within the generic categories we have already considered.

\begin{SimpleNColTable}{tab:riskTable}{5}{Project risk table}[lRRRR]
...
\begin{question}[subtitle={Activity: Risk assessment for your project }] Complete your project risk assessment by making your own version of~\Cref{tab:riskTable} and filling it in.
...
You should write a report at the end of each stage of our framework to consolidate the work you have carried out, regardless of whether your course may require you to do so. Writing such reports will help you develop your dissertation incrementally, and provide good practice to improve your academic writing skills as you go along.

\section{Putting your report together}\label{sect:Stage1report}

We recommend you use the structure and guidance in~\Cref{stage1WritingOutcomes} to write your report\footnote{Unless your course requires a different structure, which you should apply instead.} --- subsequent reports will build on this structure by adding further elements. Although not all what you write here will end up in your actual dissertation, most of it will. You've definitely started now and that has to feel good!
...
\begin{itemize}
	...
	\item In this initial stages is very important to think critically about your intended research from a number of perspectives, primarily whether it is fit-for-purpose and feasible. Contributing to this is your assessment of possible project risk and your initial planning of your work
...
In the next activity you will use~\Cref{tab:criteriaForFirstReport} to assess whether your report is of good standard. Even if you are not required to submit a report at this stage of your project, a good report will give you a solid base on which to build and report your next research increment. Now and in the long run, will save you time and effort in writing up your dissertation.
