...
In this case too you will need to iterate back to searching bibliographical databases for new hits. Alongside changing search terms, you can also make good use of the few hits you already have by apply two complementary searching techniques:  forward and backward reference searching
\textbf{LR- write about the two techniques}
...
A summary-comparison matrix\footnote{This term is used in Sastry and Mohammed (2013), although another terms used online is 'literature matrix.'} (SCMatrix) is a useful tool to track the content of academic articles in a way which also makes it easier to compare the articles you have read. 

A SCMatrix is a table which includes a row for each article you have reviewed, and a number of columns corresponding to key aspects of the articles you wish to summarise and compare with other articles, such as the research problem or question addressed, contextual facts known at start of the reported study,  key contributions made, research methods applied, etc. The exact form of the matrix can vary, but we recommend you start with the header in Table~\ref{tab:SCMatrix}, although you should feel free to adapt the matrix to your own needs, by altering the columns or adding new ones. 

\begin{table}
	\caption{Recommended header for a summary comparison matrix\label{tab:SCMatrix}}
	\begin{ltabulary}{\tablewidth} {@{}LLLLLL@{}} \toprule
...
\begin{example}{An CSMatrix example}
...
\end{itemize}
We conducted a literature search, out of which we identified eleven potentially relevant articles. We constructed a SCMatrix, whose  first three entries are reproduced in Table~\ref{tab:SCMatrixEntries}. As you can see, our entries are quite detailed. \textbf{LR: Perhaps include a screenshot? Tables in Latex are a pain!}
\end{example}
...
\begin{example}{An CMatrix example}
Carrying on with our example, we have highlighted a number of concepts in our SCMatrix, as shown in Figure X, from which we have constructed the CMatrix in Figure Y. \textbf{LR -- to decide how to include big tables...}.
...
Figure~\ref{fig:conceptMap} illustrates the three steps applied to our example: although we illustrate the final outcome, it did take us several attempt to identify a set of themes we were happy with. \textbf{LR-figure to be included.} Note how some groups are larger than others: the group 'size' provides an indication of which themes feature more prominently in the articles reviewed. For instance, in our example, lots of content is dedicated to ''modelling student trajectories'' and the ''benefits of CA tools'', while only one article deals with the 'student voice'. This is also useful as a measure of which themes you have already addressed comprehensively and which may deserve further consideration through further literature search, reading and assimilating.
