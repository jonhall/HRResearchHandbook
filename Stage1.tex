...
As the number of papers you read grows, it will be increasingly difficult for you to keep track of what each article is about without continually having to refer back to your annotations. Even more difficult will be to compare and contrast quickly the key features of different articles.

To help you with these tasks, a useful tool is to use a literature matrix\footnote{Or summary-comparison matrix (Sastry and Mohammed, 2013 (add \textbackslash{}cite).} -- a table which includes a row for each article you have reviewed, and a number of columns corresponding to key features you wish to summarise and compare across articles, such as the research problem or question addressed, contextual facts known at start,  key contributions made, research methods applied, etc. 
...
Let's look at an example.
 ...
 for each of   which a summary-comparison matrix (Sastry and Mohammed, 2013 (add \textbackslash{}cite)), SCMatrix, a basic form for which is shown in Table 2\footnote{Note that an SCMatrix must adapt to your needs as they change. Table 1 is a good starting point, but you'll probably add further columns as you progress.}. In an SCMatrix you should record the key features of each article, such as the research problem addressed, the key contribution made, the research methods applied, etc.
