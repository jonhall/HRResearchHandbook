...
To understand an academic paper is to understand the contribution to knowledge that it makes. While you won't have to understand in depth everything you read, there will be some articles which are so fundamental to your research that you will need to study them carefully. This will require a substantial investment of your time and can be very challenging: even the most experienced researcher is unlikely to be able to read an academic paper once and immediately understand it in its entirety. 
 ...
 In assimilating and analysing the content of articles you will move closer to address the following questions in relation to your research problem:
...
A summary-comparison matrix\footnote{This term is used in Sastry and Mohammed (2013), although another terms used online is 'literature matrix.'} (SCMatrix) is a useful tool to track the content of academic articles, which also makes it easier to compare the articles you have read. In essence, this is a table including a row for each article you have reviewed, and a number of columns corresponding to key concepts you wish to summarise and compare across articles, such as the research problem or question addressed, contextual facts known at start of the reported study,  key contributions made, research methods applied, etc. The exact form of the matrix can vary, but we recommend you start with the columns suggested in Table~\ref{tab:SCMatrix}, although you should feel free to adapt the matrix to your own needs. 
...
\begin{ltabulary}{\tablewidth} {@{}LLLLLL@{}} \toprule
 \textbf{Reference} & \textbf{Research problem\slash question} & \textbf{Research methods\slash approach} & \textbf{Known facts\slash assumptions\slash definitions\slash gaps at start} & \textbf{Key findings\slash contribution made} & \textbf{Notes in relation to own research} \\
 \midrule
Keshav, S. (2007) `How to read a paper', ACM SIGCOMM Computer Communication Review, 37(3), pp. 83-84. Revised in 2013. & to define an approach to reading and assimilating academic articles & the approach is explained and exemplified; no formal evaluation is presented & a practical approach to help novice researchers engage with the content of an academic article effectively and efficiently & a simple, yet effective approach, well worth applying when considering the literature\\
\bottomrule
	\end{ltabulary}
\end{solution}
\end{question}
...
What to include in your summary-comparison matrix is, of course, a matter of judgement, so there is an element of subjective interpretation of each article content. This is expected, so that the column headings we have chosen should help you be consistent and systematic across different articles. It is also up to you to choose the level of detail for each entry: while you should try to be succinct, it is important that you include enough information for you to use the matrix without having to continually refer back to your notes and articles. These may result in quite a bit of text included in the matrix, as shown in the next example, which we will use to illustrate the techniques introduced in this section.

\begin{example}{A summary-comparison matrix...}
We are interested in reviewing the literature in relation to the following research problem: \textbf{LR: table to fix!}
\begin{ltabulary}{\tablewidth}{@{}LL@{}} \\
 \textbf{Context} & Higher Education Institutions (HEIs) collect vast amount of curriculum data which could be harnessed to support curriculum decision making. While data collection, and data technology and know-how are growing across all sectors, embedding data in HEIs’ curricular decision making remains problematic \\
\textbf{Phenomena} & curriculum data; curriculum decision making processes; data technology and know-how; enablers and barriers to embedding data in curricular decision making \\
 \textbf{Knowledge gap} & how to leverage curriculum data effectively in HEIs to augment curricular decision making capability \\
 \textbf{Stakeholders} & HEIs, students, society\\
 \textbf{Reasons} & THEI’s key role is to develop and deliver curriculum to educate and exchange knowledge. It’s vital to HEIs that their curriculum is fit-for-purpose and continuously adapts to the changing needs of students and society. Data can be use as an effective tool for measuring and predicting, in support of curricular decision making \\
\end{ltabulary}
and conducted a literature search out of which we identified 11 potentially relevant articles. We constructed a summary-comparison matrix, with the first three entries illustrated in Table~\ref{tab:SCMatrixEntries}. As you can see, our entries are quite detailed. \textbf{LR: Perhaps include a screenshot? Tables in Latex are a pain!}
\end{example}
...
As the number of articles you read grows, you may find recurrent concepts among them. These might be of many types:
