...
Academic arguments are at the core of any academic writing. But what exactly is an academic argument? One of our favourite models is that proposed by Booth, Colomb and Williams (1995)\textcite{Booth1995}\footnote{We will refer to this as the BCW model.} who suggest that an academic argument has at least 2 parts – the \textit{claim} and the \textit{evidence} – and might, in complex arguments, have up to 3 others – the \textit{reason}, the \textit{warrant} and any \textit{qualifications}\footnote{Qualifications can also have the form of academic arguments, and so lead to nested arguments}. Their model is shown in Figure~\ref{fig:argument}, and the parts are:
%
%
...
A fully developed BCW argument is a beautiful thing. Here's an example\footnote{Adapted from \textcite{BCW1995}.}

{\color{red}Needs revisiting}
\begin{example}{The need for a TV `watershed'}
%
...
Qualifications can improve a claim – reducing its scope, for instance, making it more acceptible – a qualification may also require further argument. For instance, a rebuttal to \enquote{Violence is a normal part of day to day life} might be the counterclaim that \enquote{To reduce violence in day to day life, we should insulate children from it so that the next generation isn't so likely to see violence as a justified response to problems.} and so to other arguments. These might be structured the same way until all qualifications are discharged to your satisfaction.
...
\begin{question}[subtitle={Activity: Constructing academic arguments}] Consider your current theme analysis table. Consider some key points you wish to include in your literature review, and try to express them following Booth et al.'s structure.
...
Given an argument in this or any other form, the real skill of writing it down is that of producing a narrative that combines all components together in prose that is easily digestible\footnote{Meaning that your reader can easily engage with it.}.

And this leads us onto...

{\color{red} Link from argument form to lit rev isn't clear to me. Are we saying that the literature review contains all the arguments of the dissertation (``no'') or that arguments throughout the diss should be correctly formatted (yes). In the latter case, why are we using the lit rev as the place to do this?}
...
\begin{figure}[htbp]
\includegraphics[width=0.5\textwidth]{pacman}
\caption{Pacman: the relationship between the literature and the research gap.{\color{red} Move to stage 1?}}	
\end{figure}
