...
Planning work &5\% &{Refine your project plan by detailing tasks, milestones and deliverables for this stage\\ Record progress} & Appropriateness of work plan and progress\\
...
\Cref{stage2WritingOutcomes} gives you the writing outcomes for this stage. As before, they are organised around the expected structure of your final dissertation, with the activities in this part of the book designed to help you reach them.

Remember that the first column of the table gives you the expected full structure of the dissertation. Within that column: the greyed out parts are yet to be written, while those highlighted with red bullet points are to be written during this stage. The remaining parts are those you wrote in Stage 1: depending on your work in this stage, you may need to revise or adjust them slightly.
...
It's time to refine your project work plan to include a more detailed plan of your work in Stage 2. For this you will need to:

\begin{itemize}
	\item make sure you have complted all the work for Stage 1 or make adjustments 
...
You must be able to draw links between the articles that you have found, a process you started in Stage 1 by constructing a summary-comparison matrix\footnote{Refer back to\Cref{sect:Stage1AssimilatingAndAnalysing} if necessary.} and identifying emerging themes.
...
\begin{SimpleNColTable}{tab:kindOfAcademicWriting}{4}{Types of academic writing (adapted from \textcite{Cottrell2005}\color{red} is this Cotrell2023?)}[X[1]X[1]X[1]X[1]]
Descriptive & Analytic&Synthetic & Evaluative \\

 Describes what happens or what something is like or how something works & Explains why things happen or work the way they do & Makes comparisons & Identifies significance \\
 Tells a story or the order in which things occur & Shows relations between pieces of information, parts of a system & Gives reasons for choices & Demonstrates relevance \\
 Indicate how to do something & Demonstrates how a theory works & Structures information based on established criteria & Draws conclusions \\
 States what a theory says  & &  & Weighs pieces of information against one another \\
 Lists things, alternatives and options, etc. &  & & Highlights strengths and weaknesses \\
 Describes a system or its components &  & & Considers wider implications \\

\end{SimpleNColTable}

\begin{example}{Revisiting the academic writing in our theme summary}
In Stage 1, we produced the following summary around the theme of modelling learning trajectories with curriculum analytics. This summary uses a mix of all the kinds of academic writing indicated in Cref{tab:kindOfAcademicWriting}. You will consider the details in the next activity.
\begin{quotation}
...
\begin{question}[subtitle={Activity: Distinguishing between different types of academic writing}] Consider the summary above. Write down examples of each kind of academic writing you can find with reference to Cref{tab:kindOfAcademicWriting}.
\begin{solution}
...
\section{Core practice for academic writing}\label{sect:CorePracticeFor}
...
Your writing should be based on well-formed arguments, i.e., on claims that are supported by evidence\footnote{Either that of others or that you have discovered yourself. We look at academic arguments in detail in Cref{ssect:ArgumentsAndNarrative}.}, on comparing and evaluating alternative arguments, and on forming judgements on the basis of that evidence. Your writing will therefore favour analysis, synthesis and evaluation over descriptions.
...
Another form of signposting is at the level of an individual argument, when appropriate words or phrases (see Cref{tab:signalWords}) are used to help the reader understand where they are in an argument.

\begin{SimpleNColTable}{tab:signalWords}{2}{\enquote{Signal words} used in academic writing, inspired by \textcite{Cottrell2017}\color{red} is this Cottrell2023?}
Function & Possible word or phrase to use \\
...
\end{itemize}
In addition, there should be a short statement declaring that no part of the dissertation has been submitted for any other qualification.
\end{enumerate}
\end{solution}
\end{question}
%%Hack to correct tcbox behaviour
...
If you follow these simple practices consistently, you will avoid most unintentional plagiarism and your work will more likely comply with your course requirements and expectations. 
...
\begin{tip}
The guidance we give above is much easier to implement at the time of creation than at the end of the writing up process. Imagine not knowing where a quote on page 35 came from when you're writing up and the difficulty you could have in trying to find it retrospectively.
...
The development and presentation of academic arguments forms the core of academic writing. But what exactly is an academic argument? To answer this question, we look at the model proposed by Booth, Colomb and Williams in \textcite{booth1995making}; we will refer to this as the BCW model. 
...
A fully developed BCW argument is a beautiful thing. Here's a short example\footnote{Adapted from \textcite{booth1995making}.}
...
\item [Evidence] \textcite{smith1997understanding}\todo[inline]{check citation} found that children ages 5–7 who watched more than three hours of violent television each day were 25 percent points more likely to say that what they saw on television was \enquote{really happening}
...
\item [Warrant] if we understand which curriculum characteristics prevent students from succeeding on a programme of study, then we could re-design the curriculum to address the problem 
...
\begin{itemize}
	...
	\item false dichotomy\footnote{AKA black and white fallacy or false dilemma.}, in which an either--or claim is constructed which assumes that there are two mutually exclusive phenomena where that's not the case --- the phenomena may not be mutually exclusive at all. 
\end{itemize}
...
\begin{itemize}
	...
	\item \textit{post hoc ergo propter hoc}: Italy used to loose in the rugby Six Nations tournament till I started to watch them play. Therefore, my watching is causing them to win (assumed causal relationship).
