...
To make a contribution to knowledge we do research. Practically, to do research, we combine a number of research tasks into a framework. The framework is indicated by the research area, the type of contribution we wish to make, and the difficulties we face along the way.
...
%
\begin{description}
\item [Ontology] is the study of existence and addresses the question: \enquote{What is the reality that I will research?}. Practically, ontology is about finding out what you aim to examine which, in its simplest form, reduces to determining what \emph{entities} exist in the context of the your research, the \emph{relations} that exist between them and how they group together into \textit{categories}.\footnote{If you're interested, there's a fuller discussion of \emph{Ontology} in the Stanford Encyclopedia of Philosophy \parencite{hofweber2023logic}.}.

\item [Epistemology] is the study of knowledge and addresses the question: \enquote{How will I gather the knowledge and from what sources?}. Practically, epistemology is about finding out \enquote{What people know?}, \enquote{What does it mean to say that people know something?}, \enquote{What makes justified beliefs justified?}, and \enquote{How do people know that they know?}\footnote{If you're interested, there's a fuller discussion of \emph{Epistemology} in the Stanford Encyclopedia of Philosophy \parencite{steup2020epistemology}.}.

\item [Methodology] which is the system in which you choose to investigate, measure, and analyse your research’s aims and objectives. Methodology answers the \enquote{how} question, \enquote{How will I make my contribution to knowledge?}\footnote{Even if you are interested, there is (at the time of writing) no Stanford Encyclopedia entry for methodology, unfortunately. But, unlike ontology and epistemology, we go into more detail of methodology below.}.
\end{description}
...
Methodology, on the other hand, is something we will spend some time on, or at least a subfield of methodology, that dealing with how individual research methods combine to produce knowledge contributions. 

Methodology has, or can have, much more than this narrow focus, of course, often meaning the study of research methods, \emph{per se}, including questioning the assumptions that underpin their creation and application. 

Wikipedia says\footnote{It could almost be seen as a warning!}:
%
...
\subsection{Your researcher mindset}

Depending on your background, you may have begun your research studies with a particular mindset – as a scientist, a social scientist, or someone embedded within an organisation. This will flavour your approach to research. 

That's not a bad thing – many have been there before and constructed \emph{research paradigms} that characterise useful mindsets for researchers to have. There are four well-known research paradigms:

[ADAPTED from https://proofed.com/writing-tips/the-four-types-of-research-paradigms-a-comprehensive-guide/
\begin{description}%[start=0,label={(\bfseries R\arabic*):}]
\item [The Positivist Research Paradigm] which assumes that there is a single, objective reality that can be accurately known, described and explained. Positivists depend on observation of this reality to gain knowledge of it.

In this singular objective reality, researchers can compare their claims and ascertain the truth. This means researchers are limited to data collection and interpretations from an objective viewpoint.

This research paradigm is mostly used in natural sciences, physical sciences, or whenever large sample sizes are being used and leads the researcher to quantitative tasks such as, for instance, statistics, social surveys, and structured questionnaires, and towards simulations, as well as mathematical and computational thinking at the more formal end.

\newcommand{\blah}[0]{blah, blah, blah, ...}

Positivism\todo{From \url{https://image.slidesharecdn.com/lecture21-111207045819-phpapp02/95/research-paradigms-20-728.jpg?cb=1415227903}} leads to knowledge as explanations constructed from hypotheses thus established as laws or facts. As an example, think of Newton's explanation of the action of forces on matter that is encoded as his 3 Laws of Motion, for instance.

Apropos Newton's Laws, with so much invested in them, you might see that Einstein's insight into the intimate connection between space and time together with other observations taht contradicted Newton, could lead to something of a crisis in the Positivist movement~(see, for instance, \textcite{lakatos2014falsification})%, with \textcite{kuhn2012structure}
...
For instance, social science research strategy could be as simple as a two group experiment


\begin{figure}[h!]
\centering{
  \includegraphics[width=0.3\columnwidth]{marczyk2005essentialsfigp128.jpg}
  \caption[Two-group experimental design]{The two-group experimental design involves just random assignment ($R$), interventions on groups 1 and 2 ($X_1$, $X_2$), and a post-test observation ($O$)~\parencite[, adapted p.~128]{marczyk2005essentials} {\color{red}redraw}
  }
  \label{fig:marczyk2005essentialsfigp128}
  }
\end{figure} 

while the strategy\todo{Andrews calls it a research model.} described in \textcite[p.~407]{andrews2005place} for education research has 12 components, arranged in complex interacting feedback loops, with the of including experiments.

%\end{document}

\begin{figure}[hbtp]
\centering{
  \includegraphics[width=0.4\textwidth]{andrews2005placefig1.jpg}
  \caption[A complex research strategy for education research]{A complex research strategy for education research~\parencite[, Figure 1]{andrews2005place}
  \label{fig:andrews1}
  	}  
  }
\end{figure}

Although each research paradigm is sufficiently distinct as to indicate different strategies, strategies do overlap in their application. Every strategy will, for instance, to generate data of some form, whether this is readings in some experimental setting or the lived experience of a community under focus. Research data provide a focus for analysis and, eventually, synthesis of new knowledge.
...
The bad news is that there are many possible tasks you could perform at any point.

The good news is that, for your particular area in masters research, there will likely be only a small subset that you need to know about.
...
The evaluative drivers of research stem give you targets to aim at through your research; evaluation outcomes – including knowledge contribution, conclusions, and future directions for further research – will be the driver for writing up. Evaluation outcomes tell you what will be expected of your dissertation.
...
Being able to assert that you have made a contribution to knowledge is the point of structuring your research through a well-known research strategy. Even having made your assertion, however, you still need to defend it in your dissertation. That means considering, essentially, everything that could have gone wrong with the strategy – any weaknesses in the research strategy – and making arguments to patch those weaknesses so that someone scrutinising your assert contribution won't be able to fail you.  

There are four classes of weakness in all claimed knowledge contributions:
%
\begin{itemize}
...
There are three ways to address each potential weakness:
%
\begin{itemize}
\item address the weakness, i.e., being aware of the weakness during the research and putting in place research that counters it. This might be, for instance, a second of further iteration of the research strategy which addresses discovered weaknesses in earlier research.

\item avoid the weakness, i.e., choose a research strategy for which that weakness doesn't exist by, for instance, or by learning from prior research by another whose weaknesses are known and patched.

\item acknowledge and defer, most usually at the end of the research period when the research is complete, i.e., write a reflection on effect the weakness had on the outcomes and commit to addressing that weakness in future research.
\end{itemize}
%
...
%
\subsection{In detail: validity and reliability}

Reliability and validity\footnote{Adapted AI summary of \textcite{roberts2006reliability}.} are important – linked – concepts in research that serve to demonstrate the rigour and trustworthiness of both quantitative and qualitative research. Thus, researcher planning of and reflection on reliability and validity and its reporting should be a part of all research strategies.

Having said that, their form – and effort needed by you – will vary depending on the research strategy chosen. For logical proof, for instance, reliability and validity can be determines within the community of logicians within which you work, or even a computerised proof checker. At the other extreme, in grounded theory, for instance, reliability and validity may form large component of the research.
...
\paragraph{Internal validity} refers to the extent to which the study design and procedures ensure that the observed effects are actually caused by the independent variable. For a conclusion to be internally valid, you need to be able to rule out other explanations (including control, extraneous, and confounding variables) for your results. 

\paragraph{External validity} is the extent to which you can generalise the findings of a study to other situations, people, settings, and measures. In other words, can you apply the findings of your study to a broader context?

In qualitative studies, external validity is also referred to as \emph{transferability}.

External validity breaks down further into population validity and ecological validity.

\paragraph{Population validity} refers to whether you can reasonably generalise the findings from your sample to a larger group of people (the population). Population validity depends on the choice of population and on the extent to which the study sample mirrors that population. Population validity is established by showing that the sample and the population are similar.

\paragraph{Ecological validity} refers to the extent to which the measures measures how generalisable experimental findings are to the real world, such as situations or settings typical of everyday life. Ecological validity is demonstrated by showing that the restricted research context sufficiently \enquote{mimicked} the real world.

\paragraph{Construct validity} refers to the extent to which the measures used in the study accurately capture the underlying constructs or concepts of interest.

\paragraph{Content validity} refers to the extent to which the measures used in the study accurately capture the underlying constructs or concepts of interest.

\paragraph{Criterion validity} refers to the extent to which the measures used in the study accurately capture the underlying constructs or concepts of interest.

\paragraph{Concurrent validity} refers to the extent to which the measures used in the study accurately capture the underlying constructs or concepts of interest.

\paragraph{Discriminant validity} refers to the extent to which the measures used in the study accurately capture the underlying constructs or concepts of interest.

\paragraph{Face validity} refers to the extent to which the measures used in the study accurately capture the underlying constructs or concepts of interest.

\paragraph{Convergent validity} refers to the extent to which the measures used in the study accurately capture the underlying constructs or concepts of interest.

\paragraph{Predictive validity} refers to the extent to which the measures used in the study accurately capture the underlying constructs or concepts of interest.
...
There are a number of recognised threats to validity upon which an evaluation of quantitative or qualitative research validity is made.
%
\begin{itemize}
...
\paragraph{Reliability} refers to the trustworthiness and consistency of the procedures and data generated in research. It is concerned with the extent to which the results of a study or measure are repeatable in different circumstances. Reliability can be demonstrated through methods such as inter-rater reliability, test-retest reliability, and internal consistency.

Researchers use various methods, such as pilot studies and comparison to other validated measures, to establish reliability and validity. Computerized data analysis packages can also enhance reliability. It is important to balance standardization with maintaining the context and meaning of the data.
...
Triangulation\footnote{Adapted AI summary of \textcite{mathison1988triangulate}.} is a strategy used to increase the validity of evaluation and research findings. It involves using multiple data sources and methods to arrive at a singular proposition about the phenomenon being studied. However, it is important to recognise that triangulation can also result in convergent, inconsistent, and contradictory evidence. Therefore, researchers and evaluators must make sense of these different findings. Triangulation is necessary to withstand critique by colleagues and enhance the credibility of research. It involves using multiple methods, data sources, and researchers. Triangulation is a relatively new concept in social science, first introduced in \textcite{campbell1959convergent}.

Triangulation can result in inconsistent or contradictory findings, which may pose risks to the validity of research or evaluation findings. It is important to understand that triangulation does not guarantee convergence on a single proposition about a social phenomenon. Instead, it provides a rich and complex picture that requires careful interpretation and explanation by the researcher. Triangulation should be used cautiously and researchers should be prepared to explain and make sense of the various outcomes it may produce.

\begin{itemize}
\item Triangulation is a strategy for increasing the validity of evaluation and research findings: [to determine relevance, justify, and expand if necessary]
\item  It involves using multiple data sources and methods to reach a singular proposition about the phenomenon being studied.
\item  However, this expectation is unrealistic and triangulation often leads to convergent, inconsistent, and contradictory evidence.
\item  Triangulation helps to enhance the validity of research findings and withstand critique by colleagues.
\item  It is necessary to use multiple methods and sources of data in a study regardless of the philosophical or methodological perspective.
\item  Triangulation requires the researcher to make sense of the convergent, inconsistent, and contradictory evidence.
\item  Triangulation involves different types such as data triangulation, investigator triangulation, and methodological triangulation.
\item  Triangulation is a new concept in social science dating back to a paper published in 1959.
\item  Triangulation improves research practice by aiding in the elimination of bias and allowing the dismissal of rival explanations.
\end{itemize}
%

Definition of Triangulation\footnote{Human Summary of \textcite{denzin1978research}.}: ``the combination of methodologies in the study of the same phenomenon''
...
\paragraph{Investigator triangulation} involves more than one investigator in the research process, is also considered good practice. This perhaps more than other types of triangulation is usually built into the research process because most studies simply require more than one individual to accomplish the necessary data collection. However, the decision about who these multiple researchers should be and what their roles should be in the research process is problematic (Denzin, 1978; Miles, 1982). How much hands-on data collection the principal investigator needs to do in order to analyse the data, and how much data analysis is relegated to field workers because much of the analysis occurs as data are collected, are both relevant and not easily answered questions.

\paragraph{Methodological triangulation} is the most discussed type of triangulation and refers to the use of multiple methods in the examination of a social phenomenon. Psychologists have long used Denzin's notion of within-method triangulation in assessing psychological traits. Multiple scales comprise a psychological assessment such as an intelligence test in an effort to assess the different aspects of intelligence. The lie detector scale in some psychological inventories is another example. Denzin suggests that the within-methods triangulation approach has limited value, because essentially only one method is being used, and finds the between-methods triangulation strategy more satisfying. Other researchers seem to follow this lead and focus primarily on between methods triangulation. \textcquote[p.~302]{denzin1978research}{The rationale for this strategy is that the flaws of one method are often the strengths of another: and by combining methods, observers can achieve the best of each while overcoming their unique deficiences}. It is with this type of triangulation that Denzin relies most heavily on the work of Webb et al. (1966) to suggest that the use of appropriate multiple methods will result in more valid research findings.

\subsubsection{Evaluation of Triangulation} ...
...
\subsection{Your initial research strategy candidate list}
...
Without such questions and propositions, you might be tempted to cover \enquote{everything} about the individual(s), which is impossible to do.}\\
	...
	{NSC\\NEI\\NID}&Nuance from the specific context/extraneous influences/idiosyncratic details&\textcquote{kazdin1982single}{According to Kazdin(1982), the major characteristics of case studies are the following:
		\begin{itemize}
			\item  They involve the intensive study of an individual, family, group, institution, or other level that can be conceived of as a single unit.
			\item The information is highly detailed, comprehensive, and typically reported in narrative form as opposed to the quantiﬁed scores on a dependent measure.
			\item They attempt to convey the nuances of the case, including speciﬁc contexts, extraneous inﬂuences, and special idiosyncratic details.
			\item The information they examine may be retrospective or archival.
		\end{itemize}}\\ 
