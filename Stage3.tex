...
Apropos Newton's Laws, with so much invested in them, you might see that Einstein's insight into the intimate connection between space and time could lead to something of a crisis in the Positivist movement\todo{Check this was the cause, at least arguably!}, with \textcite{kuhn2012structure}. If not a complete rethink, then Einstein's work inspired a substantial away from the established \enquote{laws} and \enquote{facts}, which were neither any longer. 
...
\item describe the variants that exist and the choices that constitute it
\item describe which data collection techniques are needed
\item describe how a contribution to knowledge using the strategy will be evaluated
\item lists a number of definitive references that you give more detail.
\end{itemize}
%
...
\subsubsection{Data Generation} Suggested by the name, a survey – a standardised set of questions administered to a number of respondents – allows the researcher to gather information about a population.
...
~\parencite[p.105 (adapted)]{oates2008researching}\footnote{For each, indicate impact.}:
	%
	...
	\item Are the data generation methods used appropriate and feasible?
	\item Is the sampling frame and techniques used explained? {\color{red}Clarify \enquote{sampling frame}}
	\item Is the response rate adequate and how were non-respondents handled?
	\item Are the differences between respondents and non-respondents investigated? Are these differences significant?
	\item What additional data would have been useful?
	\item Are any generalisations made about the parent population appropriate?
	\item What limitations, flaws, errors and/or omission are there in your use of surveys?
	\item Overall, has the survey strategy been successful?
	\end{enumerate}
...
%If the outcome of reading the previous section is that you think that the survey research strategy is suitable, you might like to look into the following articles that go into more detail, either in a general setting, or in specific areas.

\resourcelistcite{dillman2014internet,oates2008researching,johannesson2014research}
\end{MoreDetails}

\subsection{Design and creation research}

The design and creation research strategy focuses on developing new solutions to problem, a problem being a need in context. The new solution can be an artefact, process, policy, model, method, anything that satisfies the need in context.

\subsubsection{Knowledge contribution}

The contribution to knowledge is that which can be learned from the creation of an artefact as the solution of a problem. Knowledge contributions therefore come from an exploration of the artefact artefact itself, or from its design, development, use, or other characteristics of the real-world problem solving process.
...
\begin{MoreDetails}
\\resourcelistcite{oates2008researching}
\end{MoreDetails}

\subsection{Experimental research}

Experimental research provides a controlled environment in which cause and effect relationships can be investigated, reducing the influence of confounding variables. From wikipedia:
...
Evaluation of the experiemntal research strategy typically consists of the following questions \parencite[p.138, adapted]{oates2008researching}%
	\begin{enumerate}%[start=0,label={(\bfseries R\arabic*):}]
...
\begin{MoreDetails}
\resourcelistcite{oates2008researching,johannesson2014research,field2002design,kalaian2008encyclopedia}	
\end{MoreDetails}
...
Case study research proceeds through the study of a single instance of the phenomenon to be investigated. The study of a single phenomenon requires the researcher to delve deeply into the context of that phenomenon, whether that be a project, an organisation, a engineered system, a policy, or any other thing.
...
Case studies come in many forms~\parencite[p.151, adapted]{oates2008researching}%
	\begin{itemize}
	\item exploratory: in which ...
	\item multiple: in which ...
	\item longitudinal: in which...
	\end{itemize}
and have possible foci \parencite[p.44]{johannesson2014research}:
	\begin{itemize} 
	\item Focus on One Instance. The idea is \enquote{To see a World in a Grain of Sand, And a Heaven in a Wild Flower, Hold Infinity in the palm of your hand, And Eternity in an hour} as expressed by William Blake (2012).
...
\begin{MoreDetails}
\resourcelistcite{oates2008researching,johannesson2014research}
\end{MoreDetails}
...
\begin{MoreDetails}
\resourcelistcite{wright2007write,moher2009preferred,pollock2018systematic}
\end{MoreDetails}
...
\begin{MoreDetails}
\resourcelistcite{smith1997understanding,corbin1990grounded,strauss1998basics,gibson2013rediscovering,charmaz2014constructing}
\end{MoreDetails}
...
\begin{MoreDetails}
\resourcelistcite{merleau1956phenomenology,anderson1991qualitative,smith2018phenomenology,shudak2018phenomenology,academic-educational-materials2019understanding,office2020the-phenomenological,groenewald2004a-phenomenological,hycner1985some}
\end{MoreDetails}
...
For mathematical and statistical modelling:
...
\begin{MoreDetails}
\resourcelistcite{dooley2017simulation}
\end{MoreDetails}
...
\begin{MoreDetails}
\resourcelistcite{Kleene1964,lakatos2015proofs,antonini2011examples}
\end{MoreDetails}
...
Having read the previous section, at this point, you've two things to do:
%
\begin{enumerate}%[start=0,label={(\bfseries R\arabic*):}]
\item use your research aims and the \enquote{Is it right for me} to choose a research strategy, and for this strategy, to
\item use the evaluation criteria to determine your results chapter.
\end{enumerate}
%
